...
//                    self.mapToWeatherInfo(weatherInfo)
    ...
    func getWeatherInfoWithCoordinates(_ lat: Double, _ long: Double) -> WeatherViewController.City {
        var city: WeatherViewController.City = WeatherViewController.City(cityID: "", name: "")
        ...
        let task = session.dataTask(with: url) {
            ...
            if let error = error {
                ...
                do {
                    let weatherData = try JSONSerialization.jsonObject(with: data!, options: .mutableContainers) as! [String: AnyObject]
                    print(weatherData)
                    city = WeatherViewController.City(cityID: "\((weatherData["city"]! as! Dictionary<String, AnyObject>)["id"] as! NSNumber)",
                                                      name: (weatherData["city"]! as! Dictionary<String, AnyObject>)["name"] as! String)
